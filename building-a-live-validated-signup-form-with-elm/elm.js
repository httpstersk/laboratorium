...
Elm.SignupForm.make = function (_elm) {
   ...
   var update = F2(function (action,model) {
      if (_U.eq(action.actionType,"VALIDATE")) {
         ...
         } else if (_U.eq(action.actionType,"SET_USERNAME")) return {ctor: "_Tuple2",_0: _U.update(model,{username: action.payload}),_1: $Effects.none};
         else if (_U.eq(action.actionType,"SET_PASSWORD")) return {ctor: "_Tuple2",_0: _U.update(model,{password: action.payload}),_1: $Effects.none};
            else if (_U.eq(action.actionType,"USERNAME_TAKEN")) return {ctor: "_Tuple2",_0: A2(withUsernameTaken,true,model),_1: $Effects.none};
               else if (_U.eq(action.actionType,"USERNAME_AVAILABLE")) return {ctor: "_Tuple2",_0: A2(withUsernameTaken,false,model),_1: $Effects.none};
                  else return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
   });
   var viewUsernameErrors = function (model) {    return model.errors.usernameTaken ? "Than username is taken!" : model.errors.username;};
   var view = F2(function (actionDispatcher,model) {
      return A2($Html.form,
      _U.list([$Html$Attributes.id("signup-form")]),
      _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Signup Form")]))
